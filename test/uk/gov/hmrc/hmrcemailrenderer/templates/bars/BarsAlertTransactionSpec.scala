...
import uk.gov.hmrc.hmrcemailrenderer.templates.bars.BarsTemplates.{ barsGroup, bars_alert_transaction }
import uk.gov.hmrc.hmrcemailrenderer.templates.{ CommonParamsForSpec, TemplateLocator }

/** @author
  *   Yuriy Tumakha
  */
class BarsAlertTransactionSpec extends AnyWordSpecLike with should.Matchers with OptionValues with CommonParamsForSpec:

  private val params = commonParameters ++ TemplateParams.exampleParams(bars_alert_transaction)
  private val template = TemplateLocator
    .templateGroups(barsGroup)
    .find(_.templateId == bars_alert_transaction)
    .value

  bars_alert_transaction should {
