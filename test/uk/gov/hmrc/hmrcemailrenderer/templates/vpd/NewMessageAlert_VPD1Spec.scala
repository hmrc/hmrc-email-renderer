...
class NewMessageAlert_VPD1Spec extends PlaySpec with CommonParamsForSpec {
  ...
  "NewMessageAlert_VPD1" must {

    val newMessageAlert: MessageTemplate = getTemplate("newMessageAlert_VPD1")

    "render correct subject" in {
      newMessageAlert.subject(commonParameters) mustBe "You’ve got a new message from HMRC"
      newMessageAlert.fromAddress(Map.empty) mustBe "Gov.uk Team <noreply@tax.service.gov.uk>"
    }

    "render htmlTemplate body and footer" in {
    ...
    "render text content" in {
      ...
      txtContent must include("You have a new message from HMRC.")
      txtContent must include("To view it, sign in to your HMRC online account.")
      txtContent must include("For security reasons, we have not included a link with this email.")
      txtContent must include("Why you got this email")
      txtContent must include("You chose to get paperless notifications.")
      txtContent must include(
        "This means we send you an email when you have a new message in your account."
      )
      txtContent must include("From HMRC digital")
    }
  }

  "NewMessageAlert_VPD1_cy welsh template" must {

    val newMessageAlert: MessageTemplate = getTemplate("newMessageAlert_VPD1_cy")

    "render correct subject" in {
      newMessageAlert.subject(commonParameters) mustBe "Mae gennych neges newydd oddi wrth CThEF"
      newMessageAlert.fromAddress(Map.empty) mustBe "Gov.uk Tîm <noreply@tax.service.gov.uk>"
    }
    ...
    "render text content" in {
      val txtContent = newMessageAlert.plainTemplate(commonParameters).toString

      txtContent must include("Mae gennych neges newydd oddi wrth CThEF")
      txtContent must include("Annwyl")
      txtContent must include("Mae gennych neges newydd oddi wrth CThEF.")
      txtContent must include("I’w gweld, mewngofnodwch i’ch cyfrif CThEF ar-lein.")
      txtContent must include(
        "Am resymau diogelwch, nid ydym wedi cynnwys cysylltiad gyda’r e-bost hwn."
      )
      txtContent must include("Pam y cawsoch yr e-bost hwn")
      txtContent must include("Gwnaethoch ddewis cael hysbysiadau di-bapur.")
      txtContent must include(
        "Mae hyn yn golygu ein bod yn anfon e-bost atoch pan fydd gennych neges newydd yn eich cyfrif."
      )
      txtContent must include("Oddi wrth adran Ddigidol CThEF")
    }
  }
}
